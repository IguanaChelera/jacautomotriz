...
@endcomponent
                ...
                <table class="table table-bordered" id="serviciosTable">
                    ...
                    <tbody>
                        ...
                        @foreach($venta->detallesServicio as $detalle)
                            @php
                                $precio = $detalle->servicio->costoServicio;
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td id="totalOrden" class="fw-bold">${{ number_format($total, 2) }}</td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Inicializa serviciosAgregados y total con los valores actuales de la tabla
            Array.from(serviciosTable.rows).forEach(row => {
                const servicioId = parseInt(row.querySelector('input[type="hidden"]').value);
                serviciosAgregados.push(servicioId);
                const subtotal = parseFloat(row.cells[3].textContent.replace('$', '').replace(',', ''));
                total += subtotal;
            });
